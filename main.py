...
# Step 1: Power check
power_on = st.radio("Is your device powered on?", ["Yes", "No"])
if power_on == "No":
    st.warning("Please power on your device and try again.")
    st.stop()

# Step 2: Restart check
restarted = st.radio("Have you tried restarting the device?", ["Yes", "No"])
if restarted == "No":
    st.info("Please restart your device and see if the problem persists.")
    st.stop()

# Step 3: Network check
network_issue = st.radio("Are you connected to the internet?", ["Yes", "No", "Not sure"])
if network_issue == "No":
